...
public class PostClassHandler implements Handler<HttpServerRequest> {
	private EventBus eb;

	public PostClassHandler(EventBus eb) {
		this.eb = eb;
	}
	...
	public void handle(final HttpServerRequest request) {
		request.bodyHandler(new Handler<Buffer>() {
			...
			public void handle(Buffer buffer) {
				...
				if (method == null) {
					handleSave(request, collection, data);
	...
	public void handleSave(final HttpServerRequest request, final String collection, JsonObject data) {
	...
	private void handleNestedRetrieve(final HttpServerRequest request, final String key, JsonObject object, final int count) {
		...
		eb.send("ssky.object", relatedOption, new Handler<Message<JsonObject>>() {
			...
			public void handle(Message<JsonObject> result) {
				...
				handleRetrieve(request, ((JsonObject) arr.get(0)).getString("className"), data);
			}
		});
	}

	private void handleInQueryRetrieve(final HttpServerRequest request, final String collection, String key, final JsonObject inMatcher) {
		JsonObject option = new JsonObject();
